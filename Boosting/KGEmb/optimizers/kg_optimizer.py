...
class KGOptimizer(object):
    ...
    def __init__(
            self, model, regularizer, optimizer, batch_size, neg_sample_size, double_neg, verbose=True, weights = None):
        """Inits KGOptimizer."""
        self.model = model
    ...
    def calculate_loss(self, input_batch, weight = None):
        ...
        if self.neg_sample_size > 0:
            ...
            predictions, factors = self.model(input_batch, eval_mode=True)
            truth = input_batch[:, 2]
            loss = nn.CrossEntropyLoss(weight = weight, reduction='mean')(predictions, truth)
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            while b_begin < examples.shape[0]:
                ...
                if self.weights == None:
                    ...
                    l = self.calculate_loss(input_batch, weight = self.weights[b_begin:b_begin + self.batch_size])
