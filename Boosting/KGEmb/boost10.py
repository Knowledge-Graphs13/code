...
#parser.add_argument(
#    "--weights", default=None, help="Weights for Cross-entropy loss"
#)
parser.add_argument(
...
def train(args, weights):
    ...
    optimizer = KGOptimizer(model, regularizer, optim_method, args.batch_size, args.neg_sample_size,
                            bool(args.double_neg), verbose = True, weights = weights)
    counter = 0
    best_mrr = None
    best_epoch = None
    ...
    return (model.get_ranking(train_examples, filters['lhs'], batch_size = args.batch_size) + model.get_ranking(train_examples, filters['rhs'], batch_size = args.batch_size))/2.0


#if __name__ == "__main__":
#    res1, res2 = train(parser.parse_args(), weights = torch.tensor([[1, 2, 3], [4, 5, 6]]))
#    print(res1.shape)

if __name__ == "__main__":
    weights = None
    for _ in range(10):
        weights = train(parser.parse_args(), weights = weights)